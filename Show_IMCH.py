...
class ThisWindow(QWidget):
    ...
    def __init__(self, n, i, question, image, filename, d, maxscore):
        super().__init__()
        self.num = n
        self.n = i
    ...
    def initUI(self):
        ...
        self.mainbox.setContentsMargins(0, 30, 0, 30)
        ...
        if len(self.question) <= 50:
            ...
            self.qtext.setFixedSize(500, 100)
        ...
        if height > 380:
            pixmap = prepixmap.scaledToHeight(260, mode=0)
        elif width <= 650:
            pixmap = prepixmap
        elif width > 650:
        ...
        self.mainbox.addSpacing(5)
        self.mainbox.addWidget(self.qtext, alignment=QtCore.Qt.AlignCenter)
        self.mainbox.addSpacing(5)
        self.mainbox.addWidget(lbl, alignment=QtCore.Qt.AlignCenter)
...
if __name__ == '__main__':
    ...
    window = ThisWindow(3, 1, 'какие предметы одежды на картинке?', 'плиссе юбка.png', 'answerfiles/Test100.txt', {'skirt': 'юбка', 'sweater': 'свитер', 'jacket': 'пиджак'}, '6')
    sys.exit(app.exec_())
